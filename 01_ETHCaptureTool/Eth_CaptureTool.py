...
class PacketCaptureApp:
    def __init__(self, root):
        ...
        self.log_file_name  = ""
        self.device_map  = {}
        self.capture_folder  = os.path.dirname(os.path.abspath(__file__))
        self.log_history  = []
        self.filter_ip  = None  # 新增：存储过滤的IP地址
        self.filter_mode  = "src"  # 新增：过滤模式(src/dst)
    ...
    def create_author_label(self):
        ...
        author_frame.grid(row=2,  column=0, sticky=tk.SE, padx=5, pady=5)
        ttk.Label(
            author_frame, 
            text="Author: gao.gao",   
            font=('Arial', 8),
    ...
    def create_version_label(self):
        ...
        version_frame.grid(row=2,  column=0, sticky=tk.SW, padx=5, pady=5)
        ttk.Label(
            version_frame,
            text="Version:1.0.3",
            font=('Arial', 8),
    ...
    # 网卡选择
        ...
        self.interface_combobox.grid(row=0,  column=1, sticky=tk.EW, pady=5, columnspan=2)
    
    # IP过滤设置（与其他输入框对齐）
        ttk.Label(main_frame, text="IP过滤:").grid(row=1, column=0, sticky=tk.W, pady=5)
    ...
    # 过滤模式选择（单选框组）
        ...
        filter_mode_frame.grid(row=1,  column=2, sticky=tk.W, padx=5)
        self.filter_mode_var  = tk.StringVar(value="src")
        ttk.Radiobutton(filter_mode_frame, text="源IP", variable=self.filter_mode_var,  value="src").pack(side=tk.LEFT)
        ttk.Radiobutton(filter_mode_frame, text="目的IP", variable=self.filter_mode_var,  value="dst").pack(side=tk.LEFT)
        ttk.Radiobutton(filter_mode_frame, text="全部", variable=self.filter_mode_var,  value="any").pack(side=tk.LEFT)
    
    # 捕获时间设置 
        ttk.Label(main_frame, text="捕获时间(分钟):").grid(row=2, column=0, sticky=tk.W, pady=5)
        self.capture_time_entry  = ttk.Entry(main_frame)
        self.capture_time_entry.insert(0,  "5")
        self.capture_time_entry.grid(row=2,  column=1, sticky=tk.EW, pady=5, columnspan=2)
    
    # 存储路径
        ttk.Label(main_frame, text="存储路径:").grid(row=3, column=0, sticky=tk.W, pady=5)
        self.path_entry  = ttk.Entry(main_frame)
        self.path_entry.insert(0,  self.capture_folder) 
        self.path_entry.grid(row=3,  column=1, sticky=tk.EW, pady=5)
        ttk.Button(main_frame, text="浏览...", command=self.select_storage_path).grid(row=3,  column=2, sticky=tk.W, pady=5)
    ...
    # 状态栏 
        ...
        self.status_label.grid(row=5,  column=0, columnspan=3, sticky=tk.EW, pady=5)
    ...
    def start_capture(self):
        ...
        self.filter_mode  = self.filter_mode_var.get() 
        
        if ip_str and self.filter_mode  != "any":
            if not self.validate_ip_address(ip_str): 
                messagebox.showerror(" 错误", "请输入有效的IP地址")
                return
            self.filter_ip  = ip_str
        else:
            self.filter_ip  = None
        ...
        # 设置BPF过滤器
        if self.filter_ip: 
            filter_str = f"ip {self.filter_mode}  host {self.filter_ip}" 
            bpf_filter = filter_str.encode() 
            bpf_program = pcap.bpf_program() 
            if pcap.compile(self.handle,  ct.byref(bpf_program),  bpf_filter, 1, 0xFFFFFFFF) == -1:
                messagebox.showerror(" 错误", "BPF过滤器编译失败")
                return
            if pcap.setfilter(self.handle,  ct.byref(bpf_program))  == -1:
                messagebox.showerror(" 错误", "BPF过滤器设置失败")
                return
        ...
        # 添加开始日志 
        filter_info = f"过滤条件: {self.filter_mode}  {self.filter_ip}"  if self.filter_ip  else "无IP过滤"
